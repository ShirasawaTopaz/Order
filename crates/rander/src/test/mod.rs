...
fn test_clear() {
    ...
    state
        .filtered_commands
        .push(("/help".to_string(), "desc".to_string()));
...
fn test_completion_partial_match() {
    ...
    assert!(
        state
            .filtered_commands
            .iter()
            .any(|(cmd, _)| cmd == "/skills")
    );
    assert!(
        state
            .filtered_commands
            .iter()
            .any(|(cmd, _)| cmd == "/settings")
    );
    assert!(
        state
            .filtered_commands
            .iter()
            .any(|(cmd, _)| cmd == "/status")
    );
}
...
fn test_completion_scroll_offset() {
    ...
    let total_commands = state.filtered_commands.len();
    assert!(
        total_commands > COMPLETION_VISIBLE_COUNT,
        "需要更多命令来测试滚动"
    );
    ...
    assert_eq!(
        selected, COMPLETION_VISIBLE_COUNT,
        "选中位置应为 {}, 实际是 {}",
        COMPLETION_VISIBLE_COUNT, selected
    );
